...
function listPedidos() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            {
                /*var textHtml = '<a class="list-group-item">' +
                                    '<div class="media">' +
                                        '<div class="pull-left p-r-10">' +
                                                '<em class="fa fa-ticket noti-primary"></em>' +
                                        '</div>' +
                                        '<div class="media-body">' +
                                            '<h5 class="media-heading"><strong>' + data[i].NombreTipoReserva + '</strong></h5>' +
                                            '<p class="m-0">' +
                                                '<small>Estado:' + 'PRE-RESERVADA' + '</small>' + 
                                            '</p>' +
                                        '</div>' +
                                        '<button>'+ '<i class="fa fa-save">'+'</i>'+'</button>'+
                                    '</div>' +
                                '</a>';*/
                var textHtml = '<div class="bar-widget" style="overflow: hidden; outline: none;" c v>' +
                                        '<div class="table-box">'+
                                            ...
                                            '<div class="table-detail">'+
                                                ...
                                                '<p class="text-muted m-b-0 m-t-0">' + '<small>Estado:' + '<br>'+'PRE-RESERVADA' + '</small>' + '</p>' +
                                            '</div>'+
                                            '<div class="table-detail text-right">'+
                                                '<a class="eliminar-reserva"  href="/Order/DeleteReserva?ReservaId=' + data[i].ReservaId + '" data-href="/Order/DeleteReserva?ReservaId=' + data[i].ReservaId + '"  data-reserva = "' + data[i].ReservaId + '">' + '<i class="fa fa-trash">' + '<label id="reservaId" style="display:none">' + data[i].ReservaId + '</label>' + '</i>' + '</a>' +
                                            '</div>'+
                                        '</div>'+
                                    '</div>'
